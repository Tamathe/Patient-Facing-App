...
> **LANDED — 2026-07-06, sprint [08 (rhtp integration)](08-rhtp-integration.md).** F4 is now built. As-built symbols other plans bind to: `classifyCrisis()` (`src/domain/safety.ts`, wrapping `src/domain/crisis-red-flags.ts`); `CRISIS_ACTIONS` / `EMERGENCY_ACTIONS` / `CARE_TEAM_ACTIONS` and the `crisis_escalate` decision kind (`src/ai/safety-gate.ts`); `AiMessageAction` widened with `crisis_call_988` / `crisis_text_988` / `call_emergency` / `safety_plan`; `AiMessage.safety` widened to `SafetyLevel` (adds `"crisis"`); `AuditEvent.action` gains `crisis_escalated`, written at the store escalation seam. `decideSafety()` runs `classifyCrisis(request.patientInput)` as its literal first statement, ahead of the stored-reading block (fixing the pre-emption noted below). Copy lives in `safetyStrings` / `tSafety` (`src/i18n/strings.ts`), en + es. Ops gate: `npm run crisis:gate` → `docs/ops/red-team-results/`.
